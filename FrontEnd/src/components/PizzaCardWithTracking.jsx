...
export function PizzaCardWithTracking({ item, quantity, onAdd, onRemove, onUpdateQuantity, availableFlavors = [] }) {
  ...
  const { trackAddToCart, trackRemoveFromCart } = useOrderTracking()
  ...
  const handleFlavorConfirm = (pizzaConfig) => {
    ...
    trackAddToCart(itemWithFlavors, 1)
  ...
  const handleAddToCart = () => {
    // Track the click on add to cart button
  ...
  const handleUpdateQuantity = (newQuantity) => {
    if (newQuantity > quantity) {
      // Adding items
      trackAddToCart(item, newQuantity - quantity)
    } else if (newQuantity < quantity) {
      // Removing items
      trackRemoveFromCart(item, quantity - newQuantity)
    }
  ...
  const handlePizzaTypeChange = (type) => {
    // Track pizza type selection
    trackClick('pizza_type_selection', {
      product_id: item.id,
      product_name: item.name,
      pizza_type: type
    })
